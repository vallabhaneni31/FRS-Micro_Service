...
import { Eye, EyeOff, Loader2, CheckCircle2, XCircle, ShieldCheck, Store } from 'lucide-react';
...
const API = (path: string) => `/api/v1/retail${path}`;

interface Props { token: string }

export const RetailInviteAcceptPage: React.FC<Props> = ({ token }) => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (!res.ok) {
        setFormError(data.error === 'invite_already_used' ? 'This invite link has already been used.'
          : data.error === 'invite_expired' ? 'This invite link has expired.'
      ...
      <div className="min-h-screen flex items-center justify-center bg-slate-50 dark:bg-[#020817] p-4">
        <div className="w-full max-w-md glass-card rounded-2xl shadow-xl border border-slate-200 dark:border-slate-700 overflow-hidden">
          <div className="bg-gradient-to-r from-slate-800 to-slate-900 px-6 py-8 text-center">
            <h1 className="text-white text-xl font-bold">FRS Retail</h1>
          </div>
          <div className="p-8 text-center">
            ...
            <p className="text-xs text-slate-400 dark:text-slate-500">
              Contact the store owner and ask them to resend the invite.
            </p>
          </div>
        </div>
      </div>
      ...
      <div className="min-h-screen flex items-center justify-center bg-slate-50 dark:bg-[#020817] p-4">
        <div className="w-full max-w-md glass-card rounded-2xl shadow-xl border border-slate-200 dark:border-slate-700 overflow-hidden">
          ...
          <div className="p-8 text-center">
            ...
            <p className="text-sm text-slate-500 dark:text-slate-400 mb-4">Taking you to the login page…</p>
    ...
    <div className="min-h-screen flex items-center justify-center bg-slate-50 dark:bg-[#020817] p-4">
      <div className="w-full max-w-md glass-card rounded-2xl shadow-xl border border-slate-200 dark:border-slate-700 overflow-hidden">
        <div className="bg-gradient-to-br from-slate-800 via-slate-800 to-slate-900 px-6 pt-8 pb-6 text-center">
          ...
          <h1 className="text-white text-xl font-bold">Set Up Your Account</h1>
          <p className="text-slate-400 text-sm mt-1">Choose a secure password to get started</p>
        </div>
          ...
          {invite && (
            <div className="bg-slate-50 dark:bg-slate-800/60 border border-slate-200 dark:border-slate-700 rounded-xl p-4 space-y-3">
              <p className="text-sm text-slate-700 dark:text-slate-200">
                👋 You've been invited to manage <strong>{invite.storeName}</strong>.
              </p>
              <div className="flex flex-wrap gap-2 text-xs">
                <span className="flex items-center gap-1 font-semibold px-2.5 py-1 rounded-full bg-blue-100 text-blue-700 dark:bg-blue-900/30 dark:text-blue-300">
                  <ShieldCheck className="w-3 h-3" /> {invite.role}
                </span>
                <span className="flex items-center gap-1 px-2.5 py-1 rounded-full bg-slate-100 dark:bg-slate-700 text-slate-600 dark:text-slate-300 font-medium">
                  <Store className="w-3 h-3" /> {invite.storeName}
                </span>
              </div>
              <p className="text-xs text-slate-400 dark:text-slate-500">
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            <div>
              ...
              <input
                ...
                className="w-full px-3 py-2.5 rounded-lg border border-slate-200 dark:border-slate-600 glass-card text-sm text-slate-900 dark:text-white placeholder:text-slate-400 focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:border-transparent"
              />
            </div>
              ...
              {password.length > 0 && (
                <div className="mt-2 space-y-1.5">
                  <div className="flex gap-1">
                    {[1, 2, 3, 4, 5, 6].map(i => (
                      <div key={i} className={cn('h-1 flex-1 rounded-full transition-all duration-300', i <= score ? STRENGTH_BAR[score] : 'bg-slate-200 dark:bg-slate-700')} />
                    ))}
                  </div>
                  <p className={cn('text-xs font-semibold', STRENGTH_COLOR[score])}>{STRENGTH_LABEL[score]}</p>
              ...
              <div className="relative">
                <input
                  ...
                  className={cn(
                    ...
                    confirm && password !== confirm ? 'border-rose-400 dark:border-rose-500'
                      : confirm && password === confirm ? 'border-emerald-400 dark:border-emerald-500'
                      : 'border-slate-200 dark:border-slate-600'
                  )}
                />
                <button type="button" onClick={() => setShowCf(v => !v)} className="absolute right-3 top-1/2 -translate-y-1/2 text-slate-400 hover:text-slate-600 dark:hover:text-slate-200">
            ...
            >
              {submitting ? (
                <span className="flex items-center justify-center gap-2"><Loader2 className="w-4 h-4 animate-spin" /> Setting up…</span>
              ) : 'Set Password →'}
            </button>
          </form>
        </div>
...
export default RetailInviteAcceptPage;
