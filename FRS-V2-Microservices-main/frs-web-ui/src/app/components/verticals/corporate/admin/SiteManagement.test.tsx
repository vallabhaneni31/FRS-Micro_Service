...
// Follow-up (AB#3233): the old hardcoded Country/State/City dropdowns are
...
async function openCreateDialog() {
  ...
  await waitFor(() => expect(screen.getByRole('heading', { name: /create new site/i })).toBeInTheDocument());

  return user;
}

async function fillRequiredFieldsWithoutPickingLocation(user: ReturnType<typeof userEvent.setup>) {
  await user.type(screen.getByPlaceholderText('Headquarters'), 'Test HQ');
  // fireEvent (not user.type) — these are plain free-text inputs now; we only
  // care about the final value, and keystroke-by-keystroke typing is slow.
  ...
  // Timezone (Radix select trigger) — the old native Country/State/City
  // <select>s are gone (now free-text inputs), so Timezone is the first
  // remaining combobox in DOM order (Status select is the second).
  const timezoneTrigger = screen.getAllByRole('combobox')[0];
...
describe('SiteManagement — precise location required to save (AB#3233)', () => {
  beforeEach(() => {
  ...
  it('blocks save when no location has been picked', async () => {
    ...
    expect(toast.error).toHaveBeenCalledWith(
      expect.stringMatching(/precise location/i)
    );
    ...
    await waitFor(() =>
      expect(apiRequestMock).toHaveBeenCalledWith(
        ...
        expect.objectContaining({
          method: 'POST',
          body: expect.stringContaining('"latitude":17.4321'),
        })
      )
    );
  ...
  it('auto-fills the editable Country, City, and Street Address fields from the picked location', async () => {
    ...
    await user.type(cityInput, 'Secunderabad');
    expect(cityInput).toHaveValue('Secunderabad');
  });
});
