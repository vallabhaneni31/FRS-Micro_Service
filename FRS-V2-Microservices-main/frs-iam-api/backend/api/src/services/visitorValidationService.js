...
import { LOGS_BUCKET } from './storageService.js';
import { checkFrameUsable } from './business/vlmQualityGateService.js';
...
// Runs `fn` over `items` with at most `limit` in flight at once. Used to run
// the VLM quality-gate calls for a batch in parallel (each call is a slow
// network round-trip) without opening 50 simultaneous requests.
...
async function processBatch() {
  ...
  // ── VLM quality-gate prefetch ────────────────────────────────────────────────
  // Run all of this batch's (slow, network-bound) VLM checks in parallel,
  // bounded to VLM_QUALITY_GATE_CONCURRENCY, before the sequential per-record
  // DB transaction loop below — keeps that loop and its promote()/SQL logic
  // completely unchanged, just fed a precomputed verdict per record.
  const vlmVerdicts = new Map(); // pk_buffer_id -> { usable, reason, checked }
  await runWithConcurrency(pending, env.vlmQualityGate.concurrency, async (record) => {
    if (!record.photo_url) {
      vlmVerdicts.set(record.pk_buffer_id, { usable: true, reason: null, checked: false });
      return;
    }
    vlmVerdicts.set(record.pk_buffer_id, await checkFrameUsable(LOGS_BUCKET, record.photo_url));
  });
  ...
  for (const record of pending) {
    ...
    try {
      ...
      if (result.status === 'employee_match') {
        ...
        // Promote to person table — withhold the photo if the VLM quality
        // gate rejected this frame (visit is still recorded normally; a
        // later good sighting of the same visitor fills the photo in via
        // promote()'s existing COALESCE(person.photo_url, EXCLUDED.photo_url)).
        const verdict = vlmVerdicts.get(record.pk_buffer_id) || { usable: true, reason: null, checked: false };
        const effectiveRecord = verdict.usable ? record : { ...record, photo_url: null };
        const personId = await promote(effectiveRecord, client);
        ...
        await client.query(
          `UPDATE visitor_buffer
           SET status = 'promoted',
               ...
               person_id = $2,
               vlm_checked_at = CASE WHEN $3 THEN NOW() ELSE NULL END,
               vlm_usable = $4,
               vlm_reason = $5
           WHERE pk_buffer_id = $1`,
          [record.pk_buffer_id, personId, verdict.checked, verdict.checked ? verdict.usable : null, verdict.reason]
        );

        logger.info(`[visitor-validation] Promoted buffer ${record.pk_buffer_id} → person ${personId}` +
          (verdict.checked && !verdict.usable ? ` (photo suppressed: ${verdict.reason})` : ''));
      }
