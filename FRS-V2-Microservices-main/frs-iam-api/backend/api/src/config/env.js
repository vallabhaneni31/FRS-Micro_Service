...
class Env {
  constructor() {
    ...
    // Self-hosted VLM quality gate for visitor photos (visitorValidationService.js) —
    // see backend/api/scripts/vlm_quality_service.py. Disabled by default: an
    // outage of this optional service must never block visitor promotion
    // (the caller fails open on any error/timeout regardless of this flag).
    this.vlmQualityGate = {
      enabled: toBoolean(process.env.VLM_QUALITY_GATE_ENABLED, false),
      baseUrl: process.env.VLM_QUALITY_GATE_URL || 'http://127.0.0.1:5065',
      // CPU inference for moondream2 measured at 20-42s/image locally —
      // 8s (a reasonable-sounding default before that was measured) timed
      // every real call out. Give real headroom above observed worst case.
      timeoutMs: toNumber(process.env.VLM_QUALITY_GATE_TIMEOUT_MS, 60000),
      concurrency: toNumber(process.env.VLM_QUALITY_GATE_CONCURRENCY, 5),
    };
