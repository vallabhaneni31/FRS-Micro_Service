-- =============================================================
-- FRS Consolidated Database Schema
-- Generated: Sat Jun 27 18:59:01 IST 2026
-- Merged from individual migration files (in execution order):
--   001_init_schema.sql
--   002_device_zone_type.sql
--   002_visitor_unknown_schema.sql
--   003_employee_site_ids.sql
--   003_visitor_enrollment_invitation.sql
--   004_attendance_ping.sql
--   004_user_lockout.sql
--   005_device_commands.sql
--   006_facility_device_column_defaults.sql
--   007_visitor_buffer.sql
-- =============================================================
...
CREATE TABLE public.frs_tenant (
    ...
    CONSTRAINT frs_tenant_vertical_check CHECK (((vertical)::text = ANY ((ARRAY['corporate'::character varying, 'education'::character varying, 'transport'::character varying])::text[])))
);
...
CREATE TABLE public.nav_item (
    ...
    CONSTRAINT nav_item_vertical_check CHECK ((((vertical)::text = ANY ((ARRAY['corporate'::character varying, 'education'::character varying, 'transport'::character varying])::text[])) OR (vertical IS NULL)))
);
...
CREATE TABLE public.scopes (
    ...
    is_active boolean DEFAULT true NOT NULL,
    created_at timestamp with time zone DEFAULT now() NOT NULL,
    CONSTRAINT scopes_vertical_check CHECK (((vertical)::text = ANY ((ARRAY['corporate'::character varying, 'education'::character varying, 'transport'::character varying])::text[])))
);
...
CREATE TABLE public.subscription_plans (
    ...
    CONSTRAINT subscription_plans_vertical_check CHECK (((vertical)::text = ANY ((ARRAY['corporate'::character varying, 'education'::character varying, 'transport'::character varying])::text[])))
);
...
CREATE TABLE public.tenants (
    ...
    CONSTRAINT tenants_vertical_check CHECK (((vertical)::text = ANY ((ARRAY['corporate'::character varying, 'education'::character varying, 'retail'::character varying, 'transport'::character varying])::text[])))
);
...
CREATE INDEX IF NOT EXISTS idx_pfe_person_id_created ON public.person_face_embeddings (person_id, created_at DESC);
CREATE INDEX IF NOT EXISTS idx_de_person_occurred ON public.device_events (fk_person_id, occurred_at DESC);
CREATE INDEX IF NOT EXISTS idx_person_updated_at ON public.person (updated_at DESC);
